...
import { FRIENDS, GIFTS, type Gift } from '~/composables/useLnData'
...
const f = computed(() => FRIENDS[active.value])
...
function sendGift(g: Gift) {
  if (ctx.coins.value >= g.p) {
    ctx.addCoins(-g.p)
    liveChat.value.push({ n: 'Bạn', t: `đã tặng ${g.em} ${g.nm}`, gift: true })
  }
}
  ...
  <div class="grid grid-cols-[290px_minmax(0,1fr)] gap-4 h-[calc(100vh-60px-3rem)] max-[720px]:grid-cols-1 max-[720px]:h-auto">
    ...
    <LnCard class="!p-0 flex flex-col overflow-hidden">
      ...
      <div class="overflow-y-auto flex-1 px-2 py-1.5">
        <button
          ...
          @click="openChat(i)"
        >
          <LnAvatar :name="fr.name" :color="fr.color" :size="38" :status="fr.status" />
          <div class="flex-1 min-w-0">
            <div class="font-body text-[0.9375rem] font-semibold">{{ fr.name }}</div>
            <div class="text-xs text-ink-3 truncate">{{ fr.status === 'offline' ? 'Ngoại tuyến' : fr.msg }}</div>
          </div>
        </button>
      </div>
    </LnCard>
    ...
    <LnCard v-else class="!p-0 flex flex-col overflow-hidden">
      <div class="flex items-center justify-between px-4 py-3 border-b border-line">
        <div class="flex items-center gap-2.5">
          ...
          <div><div class="font-body text-base font-bold">{{ f.name }}</div><div class="text-xs" :class="f.status === 'online' ? 'text-success' : 'text-ink-3'">{{ statusLabel[f.status] }}</div></div>
        </div>
    ...
    <LnDialog :open="callOpen" :width="560" @close="callOpen = false">
      ...
      <div class="grid grid-cols-2 gap-2.5">
        <div v-for="[n, c] in ([['Bạn', 'son'], [f.name, f.color]] as const)" :key="n" class="aspect-[4/3] rounded-lg-ln bg-ink grid place-items-center relative overflow-hidden">
